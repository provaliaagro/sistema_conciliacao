...
def exportar_relatorio_excel_simples(df_relatorio_conv, df_relatorio_div):
    """
                    ...
                    if sheet_name == "Transações Conciliadas":
                        if cell.value and isinstance(cell.value, str):
                            ...
                            # Estilo para cabeçalhos de tabela
                                            ...
                                            "Data Controle", "Descrição Controle", "Contraparte Controle", "Plano de Contas Controle", "Valor Controle (R$)", "Crítica"]:
                        ...
                        if cell.value and isinstance(cell.value, str):
                            ...
                            # Estilo para cabeçalhos de tabela
                            elif cell.value in ["EXTRATO", "CONTROLE FINANCEIRO", "Data", "Documento", "Descrição", "Contraparte", "Plano de Contas", "Valor (R$)", "Crítica"]:
                                cell.font = fonte_cabecalho
...
def criar_relatorio_conciliação_agrupamento(
    ...
    relatorio_dados.append([
        "Saldo Final (R$):",
        st.session_state.saldo_final_ex,
        st.session_state.saldo_final_cf
    ])
    relatorio_dados.append([])
    relatorio_dados.append([])
    ...
    operacoes_convergentes_simples = df_conciliado[
        (df_conciliado['status_conciliacao'] == 'CONCILIADA') &
        (df_conciliado['tipo_conciliacao'] != "AGRUPAMENTO")
    ]

    operacoes_convergentes_agrupadas = df_conciliado[
        (df_conciliado['status_conciliacao'] == 'CONCILIADA') &
        (df_conciliado['tipo_conciliacao'] == "AGRUPAMENTO")
    ]

    operacoes_divergentes = df_conciliado[
        df_conciliado['status_conciliacao'] == 'NÃO CONCILIADO'
    ]
    ...
    # =========================================================
    # RELATÓRIO DAS OPERAÇÕES DIVERGENTES
    # =========================================================
    ...
    if len(operacoes_divergentes) > 0:
        ...
        for _, row in operacoes_divergentes.iterrows():
            ...
            linha = {
                'data': row.get('data_extrato', ''),
                'documento': row.get('documento_extrato', ''),
                'descricao': row.get('descricao_extrato', ''),
                'valor': (
                    float(row.get('valor_extrato', 0))
                    if pd.notna(row.get('valor_extrato'))
                    else "vazio"
                )
            }
        ...
        extrato_divergente = f.ordenar_por_data_br(
            extrato_divergente
        )
        ...
        relatorio_div.append([
            "Total de Transações Não Conciliadas:",
            sum_divergente_extrato
        ])

        relatorio_div.append([
            "Valor Total das Transações Não Conciliadas (R$):",
            total_divergente_extrato
        ])
        ...
        for _, row in operacoes_divergentes.iterrows():
            ...
            linha = {
                'data': row.get('data_controle', ''),
                'recurso': row.get('recurso_controle', ''),
                'contraparte': row.get('contraparte_controle', ''),
                'plano de contas': row.get('plano de contas_controle'),
                'valor': (
                    float(row.get('valor_controle', 0))
                    if pd.notna(row.get('valor_controle'))
                    else "vazio"
                )
            }
        ...
        controle_divergente = f.ordenar_por_data_br(
            controle_divergente
        )
        ...
        relatorio_div.append([
            "Total de Transações Não Conciliadas:",
            sum_divergente_controle
        ])

        relatorio_div.append([
            "Valor Total das Transações Não Conciliadas (R$):",
            total_divergente_controle
        ])
        ...
        relatorio_div.append([
            "NENHUMA OPERAÇÃO DIVERGENTE ENCONTRADA"
        ])
    ...
    if len(operacoes_convergentes_simples) > 0:
        ...
        total_convergentes = sum(
            operacoes_convergentes_simples['valor_extrato']
        )

        relatorio_conv.append([
            "OPERAÇÕES CONVERGENTES (CONCILIADAS)"
        ])
        ...
        cabecalho = [
            ...
            "Data Controle",
            "Descrição Controle",
            "Contraparte Controle",
            "Plano de Contas Controle",
            "Valor Controle (R$)"
        ]
    ...
    if len(operacoes_convergentes_agrupadas) > 0:

        relatorio_conv.append([])
        relatorio_conv.append([])

        relatorio_conv.append([
            "OPERAÇÕES CONCILIADAS POR AGRUPAMENTO"
        ])
        ...
        for id_agr, grupo in operacoes_convergentes_agrupadas.groupby(
            "agrupamento"
        ):
    ...
    if (
        len(operacoes_convergentes_simples) == 0 and
        len(operacoes_convergentes_agrupadas) == 0
    ):

        relatorio_conv.append([
            "NENHUMA OPERAÇÃO CONCILIADA ENCONTRADA"
        ])
...
# Formatação do Relatório Excel
def exportar_relatorio_excel_agrupamento(df_relatorio_conv, df_relatorio_div):
    """
    Exporta o DataFrame do relatório para Excel em memória
    """    
                    ...
                    if sheet_name == "Transações Conciliadas":
                        if cell.value and isinstance(cell.value, str):
                            ...
                            # Estilo para cabeçalhos de tabela
                                            ...
                                            "Data Controle", "Descrição Controle", "Contraparte Controle", "Plano de Contas Controle", "Valor Controle (R$)", "Crítica", "ID Agrupamento"]:
                                cell.font = fonte_cabecalho
                                cell.fill = cor_cabecalho_conciliado
                                cell.alignment = alinhamento_centro
